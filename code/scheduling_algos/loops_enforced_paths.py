...
def attempt_solution_correct_endpoints(G: nx.DiGraph) -> Tuple[int, defaultdict, defaultdict]:
    ...
    def enforce_full_path():
        """
        """
        for stop in ends:
            G.add_edge(stop, 'night', min_trains=10)

        for stop in starts:
            G.add_edge('morning', stop, min_trains=10) 


    def remove_full_path_enforcement():
        """
        """
        G.remove_node('night')
