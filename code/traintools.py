...
line_cycler = cycler(color=colors) + cycler(
    linestyle=["-", "--", "-.", ":", "-", "--", "-."]
)
...
def minimum_number_of_trains_at_time_t(
    schedule: pd.DataFrame, train_type: Tuple[int, int]
) -> pd.DataFrame:
...
class Train:
    ...
    def plot(self) -> None:
        ...
        for index, row in self._schedule.iterrows():
            ...
            time = (
                row["departure_time"]
                - (row["departure_time"] - row["arrival_time"]) / 4
                + offset_t
            )
            ...
            ax.text(
                ...
                str(
                    (
                        ...
                        minimum_number_of_trains(
                            row, train_type=COMPARTMENT_BASED_ON_3
                        ),
                    )
                ),
                color="r",
            ...
            ax.text(
                ...
                str(
                    (
                        ...
                        minimum_number_of_trains(
                            row, train_type=COMPARTMENT_BASED_ON_4
                        ),
                    )
                ),
                color="g",
...
def create_network_schedule(
    df: pd.DataFrame, train_type: Tuple[int, int], off_set: float = 0
) -> pd.DataFrame:
    ...
    temp["trains_first"] = temp[["first_class"]].apply(
        operator.truediv, args=(train_type[0],)
    )
    temp["trains_second"] = temp[["second_class"]].apply(
        operator.truediv, args=(train_type[1],)
    )
    network_schedule["min_trains"] = temp[["trains_first", "trains_second"]].apply(
        find_bottleneck, axis=1
    )
...
def connect_stationary_nodes(G: nx.DiGraph) -> None:
    ...
    for station in nodes_per_station.keys():
        ...
        for index, stop in enumerate(nodes_per_station[station][:-1]):
            G.add_edge(
                stop,
                nodes_per_station[station][index + 1],
                min_trains=0,
                trains_needed=0,
                trains_scheduled=0,
            )
...
def graph_from_schedule(
    df: pd.DataFrame, train_type: TrainType, off_set: float = 0
) -> nx.DiGraph:
...
def find_over_scheduled(G: nx.DiGraph) -> Tuple[int, defaultdict, defaultdict]:
    ...
    starting_trains, ending_trains, number_of_trains = (
        defaultdict(list),
        defaultdict(list),
        0,
    )
    ...
    to_remove = [
        (a, b) for a, b, attrs in G.edges(data=True) if attrs["over_scheduled"] <= 0
    ]
    ...
    return (all_trains, number_of_trains, starting_trains, ending_trains, G)
...
def highlight_paths(paths: List[List[TrainStop]], G: nx.DiGraph):
    ...
    stops_at_edges = tuple(
        itertools.chain(find_ending_trainstops(G), find_starting_trainstops(G))
    )
